...
?>
    ...
    <?php $oo=mysql_fetch_array(mysql_query("SELECT*from t_jenis_pinjam where kode_jenis_pinjam='$rr'"));?>
      <?php
...
<div class="row mt">
 <div class="col-lg-12">
  <div class="form-panel"">
   ...
   <?php $dfl=mysql_fetch_array(mysql_query("SELECT *from t_anggota where kode_anggota='$kodeaa'")); echo $dfl['nama_anggota']; ?><span style="float:right;">
   <?php  $verida=mysql_fetch_array(mysql_query("SELECT * FROM t_pengajuan where kode_anggota='$kodeaa' order by kode_pengajuan desc")); $num=mysql_num_rows($verida);
  ...
  <table class="table table-bordered table-striped table-condensed">
    ...
    <tbody>
<?php
    $rino=mysql_query("SELECT * FROM t_pengajuan where kode_anggota='$kodeaa' order by kode_pengajuan desc");
  $no=1;
while($data=mysql_fetch_array($rino))
{
?>
